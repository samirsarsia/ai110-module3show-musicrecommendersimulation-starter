...
**Ranking rule (list of songs):** every song in the catalog is scored independently, then the list is built one slot at a time: at each step, the highest-scoring remaining song is picked, but its score is first reduced by **-1.0 for every artist already in the picked list who shares its artist** (the "diversity penalty," `ARTIST_REPEAT_PENALTY` in [recommender.py](src/recommender.py)). This stops the top-k from being dominated by one artist's back catalog — e.g., "Sunrise City" and "Night Drive Loop" are both by Neon Echo, so once one is picked, the other's effective score drops for the remaining slots, letting a different artist's song take its place if the scores are close. The scoring rule answers "how good is this one song for this user," while the ranking rule answers the separate question of "given many scored songs, which ones do we actually show, and in what order" — a system needs both because scoring is a per-item calculation and ranking is a list-level decision (this diversity step is a clear example of logic that belongs only in ranking, since it depends on what's *already been picked*, not on the song in isolation).
